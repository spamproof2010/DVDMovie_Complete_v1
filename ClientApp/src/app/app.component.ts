...
export class AppComponent {
  ...
  createMovie() {
    this.repo.createMovie(new Movie(0,"bit.ly/2D8C6ha", "X-Men Final Chapter", "Drama",
      "After the re-emergence of the world's first mutant, the world-destroyer Apocalypse, the X-Men must unite to defeat his extinction level plan.", 49.99, this.repo.movies[0].studio));
  }
  createMovieAndStudio() {
    let s = new Studio(0, "SkyTaylor Films", "Brooklyn", "NY");
    let p = new Movie(0, "bit.ly/2D7Vtqo", "Chef", "Romance",
      "A head chef quits his restaurant job and buys a food truck", 100, s);
    this.repo.createMovieAndStudio(p, s);
  }
  replaceMovie() {
